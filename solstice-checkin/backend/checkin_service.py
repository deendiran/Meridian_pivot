...
class PrintCompletion(BaseModel):
    job_id: str
    attendee_id: str
...
async def checkin(attendee_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Print queue unavailable; check-in can be retried.",
        ) from exc
...
async def print_complete(request: Request):
    ...
    try:
        payload = PrintCompletion.model_validate_json(raw_body)
    except ValidationError as exc:
        raise HTTPException(
            status_code=400, detail="Invalid print completion payload"
        ) from exc
